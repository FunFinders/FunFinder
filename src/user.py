...
def get_user(user_filename, user: UserModel):
    ...
    user.saved = [get_place(id) for id in user_json["saved"]]
    user.preferred_types = set(user_json["preferred_types"])
    user.liked_places = [get_place(id) for id in user_json["liked_places"]]
    user.visited = [get_place(id) for id in user_json["visited"]]
    return user
